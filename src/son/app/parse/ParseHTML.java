...
public class ParseHTML {
	...
	public ParseHTML(String newspaper, String link, Context context, int category) {
	...
	public ParseHTML(String newspaper, String link){
		this.link = link;
		this.newspaper = newspaper;
	}
	public String getContentView() throws Exception{
		...
		if (elements != null && elements.size() > 0 ){
			...
			Element element = checkImageLink(elements.first());
			content = normalizeContent(element).toString();
		} else if (getNewspaperContentId() != null){
			Element element = doc.getElementById(getNewspaperContentId());
			Log.i("id", getNewspaperContentId());
			
			element = checkImageLink(element);
			content = normalizeContent(element).toString();
			
		} else {
			content = normalizeContent(doc).toString();
		}
		return removeSize(removeAttrHeight(removeAttrStyle(removeMaxWidthImage(removeAttrWidth(content)))));
	}
	
	private Element checkImageLink(Element element){
		if (newspaper.equals("dulichvietnam") || newspaper.equals("thesaigontimes")
				|| newspaper.equals("anninhthudo") || newspaper.equals("giaoducthoidai")){
	...
	private Element removeScript(Element element){
		if (element.select("script") != null)
			element.select("script").remove();
